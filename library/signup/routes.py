...
def createUser():
    ...
    if(request.method == "POST" and form.validate_on_submit()):
        ...
        try:
            ...
            elif newUser['library_operator'].data=="Yes" and newUser['user_type'].data=="Professor":
                cur = db.connection.cursor()
                query="SELECT COUNT(*) FROM ( SELECT library_user.username FROM library_user JOIN professor ON library_user.username=professor.username  JOIN library_operator ON library_operator.prof_id=professor.prof_id  WHERE school_name=%s AND approved=true ) as subquery;"
                cur.execute(query,(newUser['school_name'].data,))
                result = cur.fetchone()[0]
                print(result)
                cur.close()
                if result>0:
                    try:
                        flash("You cannot aply for Library Operator. There is already an operator in your school.", "warning")
